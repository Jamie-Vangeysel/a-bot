...
export const PresenceUpdateEvent = {
  fire(bot: Client, config: BotConfig, member: { old: GuildMember, new: GuildMember }): Promise<any> {
    ...
    if (generalChannel && (generalChannel instanceof TextChannel)) {
      /* const embed = new RichEmbed()
        ...
        if ( member.old.presence.game ) {
          return generalChannel.send(`${member.new.displayName} is no longer playing ${member.old.presence.game.name}.`);
        }
